...
.load_metalinksdb <- function() {
  ...
  utils::data("metalinksdb", package = "scMetaLink", envir = environment())
  metalinksdb
}
...
# =============================================================================
# Spatial Transcriptomics Example Data
# =============================================================================

#' @title Colon Spatial Transcriptomics Expression Data
#' @description Example 10x Visium spatial transcriptomics expression matrix from colon tissue
#'
#' @format A sparse dgCMatrix with 4,284 genes (rows) x 1,000 spots (columns).
#'   Only genes present in MetalinksDB are included to reduce file size.
#'
#' @details
#' This is a subset of colon Visium data containing 1,000 randomly sampled spots.
#' The data is suitable for testing spatial metabolite communication analysis.
#'
#' Technical specifications:
#' \itemize{
#'   \item Platform: 10x Genomics Visium
#'   \item Spot diameter: 55 um
#'   \item Resolution: ~2.37 pixels/um
#'   \item Genes: 4,284 (filtered to MetalinksDB)
#'   \item Spots: 1,000 (subsampled from 3,313)
#' }
#'
#' @source Colon spatial transcriptomics dataset
#'
#' @seealso \code{\link{st_meta}}, \code{\link{st_scalefactors}}
#'
#' @examples
#' \donttest{
#' data(st_colon)
...
#' @title Colon Spatial Transcriptomics Metadata
#' @description Spot metadata for the colon Visium example dataset
#'
#' @format A data.frame with 1,000 rows (spots) and 5 columns:
#' \describe{
#'   \item{x}{X coordinate in image pixels (imagecol)}
#'   \item{y}{Y coordinate in image pixels (imagerow)}
#'   \item{array_row}{Row position in the Visium array}
#'   \item{array_col}{Column position in the Visium array}
#'   \item{cell_type}{Cell type annotation (mock clusters for testing)}
#' }
#'
#' @details
#' The cell_type column contains mock cell type annotations generated by
#' spatial k-means clustering. In real analysis, users should provide
#' cell type annotations from deconvolution methods (e.g., RCTD, cell2location,
#' SPOTlight) or manual annotation.
#'
#' Cell type distribution in this example:
#' \itemize{
#'   \item Epithelial: ~189 spots
#'   \item Fibroblast: ~232 spots
#'   \item Endothelial: ~174 spots
#'   \item Immune: ~145 spots
#'   \item Stromal: ~133 spots
#'   \item Tumor: ~127 spots
#' }
#'
#' @seealso \code{\link{st_expr}}, \code{\link{st_scalefactors}}
#'
#' @examples
#' \donttest{
#' data(st_colon)
...
#' @title Colon Spatial Transcriptomics Scale Factors
#' @description Scale factors for the colon Visium example dataset
#'
#' @format A list containing:
#' \describe{
#'   \item{spot_diameter_fullres}{Spot diameter in full resolution pixels (~130)}
#'   \item{tissue_hires_scalef}{Scale factor for high-res image (~0.12)}
#'   \item{tissue_lowres_scalef}{Scale factor for low-res image (~0.037)}
#'   \item{spot_diameter_um}{Spot diameter in micrometers (55 um)}
#'   \item{pixels_per_um}{Pixels per micrometer (~2.37)}
#' }
#'
#' @details
#' These scale factors are essential for converting between pixel coordinates
#' and physical distances (micrometers). The standard Visium spot diameter is
#' 55 um, which is used to calculate the pixels_per_um conversion factor.
#'
...
#'
#' @seealso \code{\link{st_expr}}, \code{\link{st_meta}}
#'
#' @examples
#' \donttest{
#' data(st_colon)
