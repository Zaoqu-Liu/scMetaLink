...
| Primary importer | MCT1 (SLC16A1) - high affinity, oxidative cells | iScience 2019 |
...
# Full analysis with both pathways
...
```{r pathway_specific, eval=FALSE}
# Direct pathway only (lactate → HCAR1)
obj <- inferLactateSignaling(obj, include_direct = TRUE, include_indirect = FALSE)

# Indirect pathway only (lactate → H+ → GPCRs)
obj <- inferLactateSignaling(obj, include_direct = FALSE, include_indirect = TRUE)
```

### Check Gene Availability

Before running analysis, check which lactate genes are available:

```{r check_genes, eval=FALSE}
...
getTopLactateProducers(obj, top_n = 5)
```
...
```{r sensing, eval=FALSE}
# Get top sensors for indirect pathway
getTopLactateSensors(obj, pathway = "indirect", top_n = 5)

# Get top sensors for direct pathway
getTopLactateSensors(obj, pathway = "direct", top_n = 5)
```

### Communication Summary

```{r summary, eval=FALSE}
# Get top cell-cell communications
getLactateSignalingSummary(obj, pathway = "combined", top_n = 10)
```
...
# Compare production and sensing across cell types
...
obj <- inferSpatialLactateSignaling(
  obj,
  max_distance = 200,      # Maximum communication distance (um)
  distance_decay = "gaussian",
  sigma = 50               # Characteristic decay distance (um)
)

# Visualize
plotSpatialLactate(obj, type = "production")

# Get hotspots
getSpatialLactateHotspots(obj, type = "all", top_n = 20)
```
