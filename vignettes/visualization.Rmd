...
```{r heatmap_basic, fig.height=8, fig.cap="**Figure 1: Basic Communication Heatmap.** Pairwise communication scores aggregated across all significant metabolite interactions. Darker colors indicate stronger communication potential."}
plotCommunicationHeatmap(obj)
```

### Customized Heatmap

```{r heatmap_custom, fig.height=8, fig.cap="**Figure 2: Customized Heatmap with Clustering.** Hierarchical clustering reveals cell type groups with similar communication patterns."}
...
```{r heatmap_metabolite, fig.height=6}
...
```{r chord_basic, fig.height=9, fig.width=9, fig.cap="**Figure 3: Communication Chord Diagram.** Ribbons connect sender to receiver cell types. Ribbon width is proportional to communication strength, and colors represent the sender cell type."}
plotCommunicationCircle(obj)
```

### Customized Chord

```{r chord_custom, fig.height=9, fig.width=9, fig.cap="**Figure 4: Customized Chord Diagram.** Enhanced visualization with adjusted transparency showing top 30 interactions."}
...
```{r network_basic, fig.height=8}
plotCommunicationNetwork(obj)
```

### Customized Network

```{r network_custom, fig.height=8}
...
```{r profile_single, fig.height=6, fig.cap="**Figure 5: Metabolite Profile.** Production (left) and sensing (right) scores across cell types for a specific metabolite."}
...
```{r profile_comparison, fig.height=8, fig.width=12}
...
```{r prod_sens_scatter, fig.height=7, fig.cap="**Figure 6: Cell Type Metabolic Roles.** Each point represents a cell type. Position shows average production (x-axis) vs sensing (y-axis) potential. Cell types above the diagonal are net sensors; those below are net producers."}
...
```{r prod_sens_bar, fig.height=6, fig.width=12, fig.cap="**Figure 7: Lactate Production and Sensing.** Comparison of lactate production (orange) vs sensing (blue) across cell types, revealing the communication axis."}
...
```{r summary_celltype, fig.height=6, fig.width=12}
sig <- obj@significant_interactions
...
```{r metabolite_dist, fig.height=6}
...
```{r custom_ggplot, fig.height=7, fig.width=9}
library(ggplot2)
...
```{r panel, fig.height=12, fig.width=14}
...
```{r palettes, fig.height=4, fig.width=10}
...
- **[Spatial Analysis](spatial-analysis.html)**: Spatial transcriptomics visualization
